<template>
  <section>
    <div class="admin-section-head"><div><p class="eyebrow">Writing desk</p><h2>文章管理</h2></div><button class="primary-button" type="button" @click="startCreate">写新文章</button></div>
    <form v-if="showEditor" class="editor-panel" @submit.prevent="savePost">
      ...
      <div class="form-actions"><button class="primary-button" :disabled="saving">{{ saving ? "正在保存…" : editingId ? "保存修改" : "创建文章" }}</button><button class="secondary-button" type="button" @click="cancelEdit">取消</button><span v-if="notice" :class="success ? 'success-text' : 'error-text'">{{ notice }}</span></div>
    </form>

    <div v-if="loading" class="admin-empty">正在读取文章…</div>
    <div v-else-if="!posts.length" class="admin-empty">还没有文章。</div>
    <div v-else class="admin-list">
      <article v-for="post in posts" :key="post.id"><img v-if="post.cover_image" :src="post.cover_image" :alt="post.title" /><div class="list-copy"><small>{{ post.category === "life" ? "生活记录" : "学习笔记" }} · {{ post.status === "published" ? "已发布" : "草稿" }}</small><h3>{{ post.title }}</h3><p>{{ post.summary || "没有摘要" }}</p></div><div class="row-actions"><button class="secondary-button" @click="editPost(post)">编辑</button><button class="danger-button" @click="removePost(post)">删除</button></div></article>
    </div>
  </section>
</template>
...
const posts = ref([]); const media = ref([]); const loading = ref(true); const saving = ref(false); const showEditor = ref(false); const editingId = ref(null); const contentInput = ref(null); const notice = ref(""); const success = ref(false);

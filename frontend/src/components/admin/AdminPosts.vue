<template>
  <section>
    ...
    <form class="admin-filters" @submit.prevent="applyFilters">
      ...
      <button class="primary-button">筛选</button><button class="secondary-button" type="button" @click="resetFilters">重置</button>
    ...
    <nav v-if="pagination.totalPages > 1" class="pagination" aria-label="文章分页">
      <button type="button" :disabled="pagination.page <= 1 || loading" @click="loadPosts(pagination.page - 1)">上一页</button>
      <button v-for="pageNumber in pageNumbers" :key="pageNumber" type="button" :class="{ active: pageNumber === pagination.page }" :disabled="loading" @click="loadPosts(pageNumber)">{{ pageNumber }}</button>
      <button type="button" :disabled="pagination.page >= pagination.totalPages || loading" @click="loadPosts(pagination.page + 1)">下一页</button>
    </nav>
  </section>
</template>
...
const pageNumbers = computed(() => { const end = Math.min(pagination.totalPages, Math.max(5, pagination.page + 2)); const start = Math.max(1, Math.min(pagination.page - 2, end - 4)); return Array.from({ length: end - start + 1 }, (_, index) => start + index); });
...
async function removePost(post) { if (!window.confirm(`删除「${post.title}」？`)) return; try { await deleteAdminPost(post.id); const targetPage = posts.value.length === 1 && pagination.page > 1 ? pagination.page - 1 : pagination.page; success.value = true; notice.value = "文章已删除。"; await loadPosts(targetPage); } catch { notice.value = "文章删除失败。"; success.value = false; } }

    ...
    <section v-else class="album-grid">
      <article v-for="album in albums" :key="album.id" class="album-card surface">
        ...
        <div v-if="album.photos?.length" class="photo-strip">
          <a v-for="photo in album.photos.slice(0, 4)" :key="photo.id" :href="photo.image_url" target="_blank"><img :src="photo.thumbnail_url || photo.image_url" :alt="photo.caption || album.title" loading="lazy" /></a>
        </div>
      </article>
    </section>
  </div>
